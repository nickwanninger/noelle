    ...
    while (!Worklist.empty()) {
      ...
      switch (I->getOpcode()) {
        ...
        case Instruction::Invoke: {
          ...
          // point, or a use of an alloca. Calling with byval copies the
          // contents of the alloca into argument registers or stack slots,
          // which exist beyond the lifetime of the current frame.
...
static bool mightHaveSideEffects(Instruction *I) {
...
static Value *canTransformAccumulatorRecursion(Instruction *I, CallInst *CI) {
  ...
  if (!I->isAssociative() || !I->isCommutative()) {
    errs() << "I is not associative or communative. Bailing from TRE\n";
    return nullptr;
  }
...
static CallInst *findTRECandidate(Instruction *TI,
  ...
  errs() << "Looking for TRE candidate from " << *TI << "\n";
  ...
  if (BB == &F->getEntryBlock() && firstNonDbg(BB->front().getIterator()) == CI
...
static bool foldReturnAndProcessPred(BasicBlock *BB,
  ...
  while (!UncondBranchPreds.empty()) {
    ...
    if (CallInst *CI =
      ...
      LLVM_DEBUG(dbgs()
                 << "FOLDING: " << *BB << "INTO UNCOND BRANCH PRED: " << *Pred);
...
static bool processReturningBlock(ReturnInst *Ret,
  ...
  if (!CI) {
    errs() << "No TRE Candidate!\n";
    return false;
  }

  errs() << "TRE Candidate: " << *CI << "\n";
...
static bool eliminateTailRecursion(Function &F,
  ...
  if (!AllCallsAreTailCalls) {
    errs() << "Not all calls are tailcalls\n";
    return MadeChange;
  }

  errs() << "All calls are tailcalls\n";

  // If this function is a varargs function, we won't be able to PHI the args
  // right, so don't even try to convert it...
  if (F.getFunctionType()->isVarArg()) {
    errs() << "The function is varargs!\n";
    return false;
  }
  ...
  for (Function::iterator BBI = F.begin(), E = F.end(); BBI != E; /*in loop*/) {
    ...
    if (ReturnInst *Ret = dyn_cast<ReturnInst>(BB->getTerminator())) {
      ...
      bool Change = processReturningBlock(Ret,
                                          ...
                                          ORE,
                                          DTU);
      errs() << "processReturningBlock returned " << Change << "\n";
...
void splitReturns(llvm::Function &F) {
  ...
  errs() << "Preparing " << F.getName() << " for TRE with splitReturn\n";
  ...
  for (BasicBlock *Pred : predecessors(BBRI)) {
    ...
    if (auto br = dyn_cast<BranchInst>(Pred->getTerminator())) {
      if (!br->isUnconditional()) {
        errs() << "One of the predecessors' terminator is not an unconditional "
                  "branch!\n";
        return;
      }
    }
  ...
  if (preds.size() <= 1) {
    errs() << "Not enough preds for the unified return block!\n";
    return;
  }

  // Ensure that the block we are splitting has a maximum of one phi instruction
  PHINode *retPhi = nullptr;

  for (auto &I : *BBRI) {
    if (auto Phi = dyn_cast<PHINode>(&I)) {
      if (retPhi != nullptr) {
        errs() << "Multiple PHI nodes. Bailing!\n";
        return;
      }
      retPhi = Phi;

      errs() << "not handling PHIs yet\n";
      return;
    }
  }
  ...
  // Now, iterate over each predecessor, copying all the instructions (including
  // the PHI, despite it being wrong).
  for (auto *pred : preds) {
    ...
    for (auto &inst : *BBRI) {
      ...
      vmap[&inst] = new_inst;
      llvm::RemapInstruction(new_inst,
                             vmap,
...
struct CAT : public FunctionPass {
  ...
  bool runOnFunction(Function &F) override {
    ...
    // TRE would like all calls to be marked as `tail call` in the IR. As far
    // as I can tell, this has no effect outside of marking the call as a
    ...
    errs() << "Running cat pass on " << F.getName() << "\n";
...
char CAT::ID = 0;
static RegisterPass<CAT> X("noelle-tailcallelim",
